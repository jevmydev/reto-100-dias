...
const GradesForm = ({ handleSubmit }) => {
    const [increment, useIncrement] = useState(1);
    const incrementChange = (e) => useIncrement(e.target.value);
...
export const GradesAside = ({ handleSubmit }) => {
    return (
        <aside className="w-1/5 h-full">
            <GradesForm handleSubmit={handleSubmit} />
        </aside>
    );
};
